 ...
 *
 *  使用方法：创建一个播放器对象
 *  var MusicPlayer = new MusicPlayer(audio播放器对象, img封面图对象, {
 *    volume:音量大小,
 *    speed:播放速度,
 *    music:音乐资源,
 *    photo:封面图资源
...
(function () {
    ...
    var coverImg;
    var monitor;
    function MusicPlayer(playerID, coverImgID, obj) {
    ...
    //获取音乐链接
    MusicPlayer.prototype.getMusicSrc = function (type) {
        switch (type){
            case 'relative':
                return this.music;
            case 'absolutely':
    ...
    //获取播放速度
    MusicPlayer.prototype.getSpeed = function () {
    ...
    //增加播放速度
    MusicPlayer.prototype.speedUp = function () {
    ...
    //减小播放速度
    MusicPlayer.prototype.speedDown = function () {
    ...
    //设置播放曲目
    MusicPlayer.prototype.setMusic = function(musicSrc) {
        this.music = musicSrc;
        this.progressPercent = '0.00%';
        return this;
    };
    ...
    //设置封面图片
    MusicPlayer.prototype.setCoverImage = function (photoSrc) {
        this.photo = photoSrc;
        return this;
    };

    //使改变的曲目在播放器中生效
    MusicPlayer.prototype.change = function () {
        mediaPlay.setAttribute('src',this.music);
        for (let i = 0; i < coverImg.length; i++){
            coverImg[i].setAttribute('src',this.photo);
        }
    ...
    MusicPlayer.prototype.setActionsAfterPlayback = function (flag, callback) {
    ...
    //暂停
    MusicPlayer.prototype.pause = function (callback) {
        this.state = false;
        mediaPlay.pause();
    ...
    //停止
    MusicPlayer.prototype.stop = function (callback) {
        this.state = false;
        mediaPlay.pause();
    ...
    //获取音乐播放状态
    MusicPlayer.prototype.getPlayStatus = function () {
        return this.state;
    };

    //监听音乐播放状态发生改变时执行的结果（回调1：正在播放，回调2：无播放）
    var playStatusMonitor;
    MusicPlayer.prototype.addPlayStatusEventListener = function (callback1, callback2) {
    ...
    MusicPlayer.prototype.setVolume = function (number, type) {
    ...
    //设置音量进度条
    MusicPlayer.prototype.setVolumeProgress = function(event, object, type){
        let currentValue;
    ...
    MusicPlayer.prototype.mute = function (isMute) {
        if (isMute){
            mediaPlay.volume = 0;
            this.isMute = true;
    ...
    MusicPlayer.prototype.addVolumeProgressEventListener = function (className, type = null) {
    ...
    //获取当前进度百分比
    MusicPlayer.prototype.getProgress = function () {
    ...
    MusicPlayer.prototype.addProgressEventListener = function (className, startTimeClassName, endTimeClassName) {
    ...
    //设置播放进度
    MusicPlayer.prototype.setPlayProgress = function (event, object) {
    ...
    //获取缓冲进度
    var bufferedMonitor;
    MusicPlayer.prototype.addBufferedProgressEventListener = function (className) {
        //进度条更新
    ...
    //获取“进度条”监听器
    MusicPlayer.prototype.getMonitorProgress = function () {
        monitor = progressMonitor;
        return this;
    };

    //获取“音量控制条”监听器
    MusicPlayer.prototype.getMonitorVolumeProgress = function () {
        monitor = volumeProgressMonitor;
        return this;
    };

    //获取“缓冲条”监听器
    MusicPlayer.prototype.getMonitorBufferedProgress = function () {
        monitor = bufferedMonitor;
        return this;
    };

    //获取“播放状态”监听器
    MusicPlayer.prototype.getMonitorPlayStatus = function () {
        monitor = playStatusMonitor;
        return this;
    };

    //清理全部监听器
    MusicPlayer.prototype.clearMonitor = function () {
    ...
    //设置缓冲导致的播放停止事件
    MusicPlayer.prototype.setBufferUnexpectedStop = function (callback) {
        if(callback){
            buStop = function () {
                if (buStopSwitch){
                    return;
